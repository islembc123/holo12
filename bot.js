...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['Ihelp | Islem BC BOT'];
    var i = -1;
    var j = 0;
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/islembc`);
    }, ms);
...
${prefix}invite/ يعرض لك رابط لدعوة البوت لسيرفرك
${prefix}new/ لفتح تكت 
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('``My ID``' , `[ ${client.user.id} ]` , true)
			      .addField('``My Prefix``' , `[ I ]` , true)
			      .addField('``My Language``' , `[ Java Script ]` , true)
			      .setFooter('By | Islem BC')
    })
}
});
 client.on('message', message => {
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Islem BC BOT ©\'s translator',
...
client.on('message', async message => {
   ...
   if (message.content.startsWith("Inew")) {  
 ...
 client.on('message', message => {
    if (message.content.startsWith("Iinvite")) {
    ...
    const embed = new Discord.RichEmbed()
        .setColor("RANDOM")
          .setDescription("https://discordapp.com/api/oauth2/authorize?client_id=605362169684557834&permissions=2146957767&scope=bot")
           .setAuthor(client.user.username, client.user.avatarURL)
                 .setAuthor(client.user.username, client.user.avatarURL)
...
client.on('guildCreate', guild => {
     ...
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=605362169684557834&permissions=2146957767&scope=bot')
  .setDescription(`**
  New Server Add Islem BC BOT © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=605362169684557834&permissions=2146957767&scope=bot')
  .setDescription(`**
  Server Kicked Islem BC BOT © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
